...
$query = "SELECT * FROM master_transportasi ORDER BY id DESC";
...
<div class="flex-grow-1 p-4 bg-light">
    ...
    <nav class="navbar navbar-light bg-white shadow-sm mb-4 rounded">
        <div class="container-fluid">
            <span class="navbar-brand mb-0 h1">Jenis Trasnportasi</span>
        </div>
    </nav>
    ...
    <div class="card shadow-sm border-0">
        ...
        <td><?= $row['kode']; ?></td>
        <td><?= $row['nama_jenis_transportasi']; ?></td> 
        <td class="text-center">
            <button class="btn btn-sm btn-warning text-white" 
                    onclick="bukaModalEdit('<?= $row['id']; ?>', '<?= $row['kode']; ?>', '<?= $row['nama_jenis_transportasi']; ?>')">
