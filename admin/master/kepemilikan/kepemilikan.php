...
$query = "SELECT * FROM master_jenis_kepemilikan ORDER BY id DESC";
...
<div class="flex-grow-1 p-4 bg-light">
    ...
    <nav class="navbar navbar-light bg-white shadow-sm mb-4 rounded">
        <div class="container-fluid">
            <span class="navbar-brand mb-0 h1">Kepemilikan</span>
        </div>
    </nav>
    ...
    <div class="card shadow-sm border-0">
        ...
        <td class="text-center">
            <button class="btn btn-sm btn-warning text-white" 
                    onclick="bukaModalEdit('<?= $row['id']; ?>', '<?= $row['kode']; ?>', '<?= $row['nama_jenis_kepemilikan']; ?>')">
                <i class="fas fa-edit"></i>
            </button>
            <a href="hapus.php?id=<?= $row['id']; ?>&type=jenis_kepemilikan" class="btn btn-sm btn-danger" onclick="return confirm('Yakin hapus data ini?')">
